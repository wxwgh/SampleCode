package com.supermap.desktop.maptool.gp;
...
import com.supermap.desktop.maptool.MapToolProperties;
...
public class DesktopProcessSampleCode extends AbstractDesktopProcess {
	...
	public DesktopProcessSampleCode() {
		...
		setTitle(MapToolProperties.getString("String_DeleteDuplicatePoints"));
	...
	public void processResult(HashMap<String, Object> result) {
		if (result.containsKey("resultDataset") && result.get("resultDataset")!=null){
			...
			Application.getActiveApplication().getOutput().output(MessageFormat.format(MapToolProperties.getString("String_DeleteDuplicatePointsSuccess"),datasetVector.getName()));
		}else{
			Application.getActiveApplication().getOutput().output(MapToolProperties.getString("String_DeleteDuplicatePointsFailed"));
		}
	}
}
