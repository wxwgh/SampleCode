 ...
 * @author SuperMap
 */
public class DialogTileLoad extends SmDialog {
    ...
    private void initPool() {
         ...
         */
        if (tilePool == null) {
            tilePool = Executors.newFixedThreadPool(MapUrl.tilePoolSize);
        }
        /**
    ...
    private void initTable() {
        if (MapUrl.mapType.equals("baidu")) {
            ...
            for (int i = 3; i < MapUrl.BaiduScale.length; i++) {
                ...
                list.add(i + 1);
                list.add(MapUrl.BaiduScale[i]);
                list.add("256x256");
                list.add(tileNum);
                list.add(tileWeight);
                this.rowData.add(list);
            }
        }else if(MapUrl.mapType.equals("google")){
            Tile tile1 = new Tile();
            Tile tile2 = new Tile();
        ...
        }else if(MapUrl.mapType.equals("tianditu")){
            Tile tile1 = new Tile();
            Tile tile2 = new Tile();
            for (int i = 0; i < MapUrl.TianDiTuScale.length; i++) {
                long tileNum = 0;
                // 经纬度转瓦片
                CoordinateToolkit.wgs84_LngLat_to_Tile(MapUrl.northwestTianDiTu, i+1, tile1);
                CoordinateToolkit.wgs84_LngLat_to_Tile(MapUrl.southeastTianDiTu, i+1, tile2);
                int minX = tile1.getX() < tile2.getX() ? tile1.getX() : tile2.getX();
                int minY = tile1.getY() < tile2.getY() ? tile1.getY() : tile2.getY();
                int maxX = tile1.getX() > tile2.getX() ? tile1.getX() : tile2.getX();
                int maxY = tile1.getY() > tile2.getY() ? tile1.getY() : tile2.getY();
                ...
                list.add(Boolean.FALSE);
                list.add(i+1);
                list.add(MapUrl.TianDiTuScale[i]);
                list.add("256x256");
                list.add(tileNum);
                list.add(tileWeight);
                this.rowData.add(list);
    ...
    private void initPanel(){
        ...
        JTextField taskNameField = new JTextField("影像地图下载",24);
        //创建固定宽度不可见组件
    ...
    private void addTask(String id) {
        ...
        //地图类型
        task.setMapType(MapUrl.mapType);
        //地图名称
        task.setMapName(MapUrl.mapName);
        //当前下载瓦片数量
        ...
        downTaskPool.execute(() -> {
            ...
            for (int i = 0; i < level.size(); i++) {
                if(MapUrl.mapType.equals("baidu")){
                    ...
                    CoordinateToolkit.wgs84_LngLat_to_Tile(MapUrl.northwestTianDiTu, (int) level.get(i), tile1);
                    CoordinateToolkit.wgs84_LngLat_to_Tile(MapUrl.southeastTianDiTu, (int) level.get(i), tile2);
                }
                        ...
                        if (isJoin) {
                            try {
                                ...
                                if (!new File(fileName).exists()) {
                                    ...
                                    if (this.isCrop) {
                                        byte[] content=null;
                                        if(MapUrl.mapType.equals("tianditu")){
                                            content = executor.execute(Request.Get(url).connectTimeout(5000).socketTimeout(5000).setHeader("User-Agent","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36")).returnContent().asBytes();
                                        }else{
                                            content = executor.execute(Request.Get(url).connectTimeout(5000).socketTimeout(5000)).returnContent().asBytes();
                                        }
                                        //将字节作为输入流
                ...
                try {
                    ...
                    if (!new File(downFile.getFileName()).exists()) {
                        ...
                        if (isCrop) {
                            byte[] content;
                            if(MapUrl.mapType.equals("tianditu")){
                                content = executor.execute(Request.Get(downFile.getUrl()).connectTimeout(5000).socketTimeout(5000).setHeader("User-Agent","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36")).returnContent().asBytes();
                            }else{
