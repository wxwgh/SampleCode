package com.supermap.desktop.download.gp;
...
public class DesktopProcessSampleCode extends AbstractDesktopProcess {
	...
	public DesktopProcessSampleCode() {
		...
		setTitle(DownLoadProperties.getString("String_DeleteDuplicatePoints"));
	...
	public void processResult(HashMap<String, Object> result) {
		if (result.containsKey("resultDataset") && result.get("resultDataset")!=null){
			...
			Application.getActiveApplication().getOutput().output(MessageFormat.format(DownLoadProperties.getString("String_DeleteDuplicatePointsSuccess"),datasetVector.getName()));
		}else{
			Application.getActiveApplication().getOutput().output(DownLoadProperties.getString("String_DeleteDuplicatePointsFailed"));
		}
	}
}
