...
import com.supermap.desktop.download.bean.Mercator;
...
public class CoordinateToolkit {
    ...
    /**
     * 大地2000坐标系转瓦片坐标系 --WGS84 天地图
     */
    public static void wgs84_LngLat_to_Tile(LngLat lngLat, int zoom, Tile tile){
        double x = (lngLat.getLng() + 180) / 360;
        double tileX = Math.floor(x * Math.pow(2, zoom));
        double lat_rad = lngLat.getLat() * Math.PI / 180;
        double y = (1 - Math.log(Math.tan(lat_rad) + 1 / Math.cos(lat_rad)) / Math.PI) / 2;
        double tileY = Math.floor(y * Math.pow(2, zoom));
        tile.setX((int) tileX);
        tile.setY((int) tileY);
        tile.setZoom(zoom);
