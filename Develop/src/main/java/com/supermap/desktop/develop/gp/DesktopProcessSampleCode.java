package com.supermap.desktop.develop.gp;
...
public class DesktopProcessSampleCode extends AbstractDesktopProcess {
	...
	public DesktopProcessSampleCode() {
		...
		setTitle(DevelopProperties.getString("String_DeleteDuplicatePoints"));
	...
	public void processResult(HashMap<String, Object> result) {
		if (result.containsKey("resultDataset") && result.get("resultDataset")!=null){
			...
			Application.getActiveApplication().getOutput().output(MessageFormat.format(DevelopProperties.getString("String_DeleteDuplicatePointsSuccess"),datasetVector.getName()));
		}else{
			Application.getActiveApplication().getOutput().output(DevelopProperties.getString("String_DeleteDuplicatePointsFailed"));
		}
	}
}
